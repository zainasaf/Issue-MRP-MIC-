...
## download.file("https://mrp-psf.ams3.digitaloceanspaces.com/models/mental_health_intervention_model.rds",
## destfile = "mental_health_intervention_model.rds", mode = "wb")

fit <- readRDS("head_ofstate_model.RDS")



try(dir.create("age_level_headstate"))
try(dir.create("edu_level_headstate"))
try(dir.create("ethn_level_headstate"))
try(dir.create("sex_level_headstate"))
try(dir.create("constituency_headstate"))
try(dir.create("age_sex_level_headstate"))
...
i=633-j
  ...
  pred <- pred %>%
    as.data.frame() %>% 
    rename_with(~str_replace(., "V", "pred"), everything()) %>%
    mutate(id = as.character(row_number()), .before = pred1) 
  pred <- full_join(data, pred, by = "id") %>%
    select(c(id, const_name, age_level, sex_level, edu_level, ethn_level, perc, starts_with("pred")))
  
  pred <- pred %>%
    mutate(across(starts_with("pred"), as.double)) 
  
  pred_ed <- pred %>%
    group_by(const_name, edu_level) %>%
    mutate(perc = perc/sum(perc), 
           across(starts_with("pred"), ~ .x * perc))    %>%
    summarise(across(starts_with("pred"), sum), .groups='drop') %>%
    rowwise() %>%
    mutate(mean = mean(c_across(starts_with('pred')), na.rm = TRUE)) %>%
    ungroup() %>% 
    select(const_name, edu_level, mean)
  ...
  pred_eth <- pred %>%
    ...
    mutate(mean = mean(c_across(starts_with('pred')), na.rm = TRUE)) %>%
    ungroup() %>% 
    select(const_name, ethn_level, mean)
  ...
  pred_gen <- pred %>%
    ...
    mutate(mean = mean(c_across(starts_with('pred')), na.rm = TRUE)) %>%
    ungroup()%>% 
    select(const_name, sex_level, mean)
  ...
  pred_age <- pred %>%
    ...
    mutate(mean = mean(c_across(starts_with('pred')), na.rm = TRUE)) %>%
    ungroup()%>% 
    select(const_name, age_level, mean)
  
  
  pred_const <- pred %>%
    mutate(across(starts_with("pred"), ~ .x * perc)) %>%
    group_by(const_name) %>%
    dplyr::summarise(across(starts_with("pred"), sum)) %>% 
    rowwise() %>%
    dplyr::mutate(mean = mean(c_across(c(starts_with('pred'))), na.rm=TRUE)) %>%
    select(const_name,mean)
  
  
  write.csv(pred_age,paste0("age_level_headstate/headstate_preds_",const_name_i,".csv"))
  write.csv(pred_ed,paste0("edu_level_headstate/headstate_preds_",const_name_i,".csv"))
  write.csv(pred_eth,paste0("ethn_level_headstate/headstate_preds_",const_name_i,".csv"))
  write.csv(pred_gen,paste0("sex_level_headstate/headstate_preds_",const_name_i,".csv"))
  write.csv(pred_const,paste0("constituency_headstate/headstate_preds_",const_name_i,".csv"))
  write.csv(pred_const,paste0("age_sex_level_headstate/headstate_preds_",const_name_i,".csv"))
  
  
  #  tictoc::toc()
}

const_files <- list.files(path="constituency_headstate/", pattern="\\.csv", full.names=TRUE)
const_dat <- lapply(const_files, \(x)read_csv(x))
const_dat_comb <- bind_rows(const_dat) %>% select(-1)
write.csv(const_dat_comb, "constituency_headstate/headstate_constituency_combined.csv")

age_files <- list.files(path="age_level_headstate/", pattern="\\.csv", full.names=TRUE)
age_dat <- lapply(age_files, \(x)read_csv(x))
age_dat_comb <- bind_rows(age_dat) %>% select(-1)
write.csv(age_dat_comb, "age_level_headstate/headstate_age_combined.csv")

edu_files <- list.files(path="edu_level_headstate/", pattern="\\.csv", full.names=TRUE)
edu_dat <- lapply(edu_files, \(x)read_csv(x))
edu_dat_comb <- bind_rows(edu_dat) %>% select(-1)
write.csv(edu_dat_comb, "edu_level_headstate/headstate_edu_combined.csv")

ethn_files <- list.files(path="ethn_level_headstate/", pattern="\\.csv", full.names=TRUE)
ethn_dat <- lapply(ethn_files, \(x)read_csv(x))
ethn_dat_comb <- bind_rows(ethn_dat) %>% select(-1)
write.csv(ethn_dat_comb, "ethn_level_headstate/headstate_ethn_combined.csv")

sex_files <- list.files(path="sex_level_headstate/", pattern="\\.csv", full.names=TRUE)
sex_dat <- lapply(sex_files, \(x)read_csv(x))
sex_dat_comb <- bind_rows(sex_dat) %>% select(-1)
write.csv(sex_dat_comb, "sex_level_headstate/headstate_sex_combined.csv")


age_sex_files <- list.files(path="age_sex_level_headstate/", pattern="\\.csv", full.names=TRUE)
age_sex_dat <- lapply(age_sex_files, \(x)read_csv(x))
age_sex_dat_comb <- bind_rows(age_sex_dat) %>% select(-1)
write.csv(age_sex_dat_comb, "age_sex_level_headstate/headstate_age_sex_combined.csv")
