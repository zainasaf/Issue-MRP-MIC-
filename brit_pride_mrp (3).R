...
### how proud or ashamed are you to be British - Very prod, quite, pround neither proud nor ashmed, quite ashamed, very ashmed 
...
miss_codes <- polldata$postcode_clean[which(is.na(polldata$constituency))]
table(nchar(miss_codes))
un_miss_codes <- unique(miss_codes)
un_miss_codes %in% 

n_const %>% filter(postcode_clean %in% un_miss_codes) %>% print(n=150)
...
polldata <- polldata %>%
  mutate(pride_numeric = case_when(
...
overall_britishpride <- polldata %>% group_by(pride_numeric) %>% count()
overall_britishpride$pct <- 100*overall_britishpride$n/nrow(polldata)
...
polldata %>%
  select(depriv_index_country, ruc_country, euref_leave, house_prices_perc, students) %>%
...
saveRDS(model_1, "british_pride_model.rds")
