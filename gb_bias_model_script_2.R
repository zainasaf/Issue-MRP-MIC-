...
## would you say that GB news has a bias in its news coverage ## 
...
polldata <- polldata %>%
  mutate(gbnews_bias_numeric_dk_sep = case_when(
...
overall_gbnews_bias <- polldata %>% group_by(gbnews_bias_numeric_dk_sep) %>% count()
overall_gbnews_bias$pct <- 100*overall_gbnews_bias$n/nrow(polldata)
...
gbnews_bias_data <- polldata


gbnews_bias_data <- polldata %>%
  mutate(
    gbnews_bias_cat = factor(
      gbnews_bias_numeric_dk_sep,
      levels = 1:6,
      labels = c(
        "very_right",
        "mod_right",
...
model_1 <- brm(
  formula = formula_1,
  data = gbnews_bias_data,
  family = categorical(link = "logit"),
  seed = 320,
  silent = 0,
  iter = iter,
  prior = prior_gbnews_bias,
  chains = chains,
...
saveRDS(model_1, "gbnews_bias_model.rds")
