...
## How likely do you think it is that, in the next couple of years, Britain will see unrest or rioting related to the issues of immigration or asylum?
...
polldata <- polldata %>%
  mutate(riotslikely_numeric_dk_sp = case_when(
    riotslikely == "Very likely" ~ 1,
    riotslikely == "Quite likely" ~ 2,
    riotslikely == "Neither likely nor unlikely" ~ 3,
    riotslikely == "Quite unlikely" ~ 4,
    riotslikely == "Very unlikely" ~ 5,
    riotslikely == "Don’t know" ~ 6,  
    TRUE ~ NA_real_
  ))
...
overall_riotslikely <- polldata %>% group_by(riotslikely_numeric_dk_sp) %>% count()
overall_riotslikely$pct <- 100*overall_riotslikely$n/nrow(polldata)
...
riots_likely_data <- polldata


riots_likely_data <- polldata %>%
  mutate(
    riots_likely_cat = factor(
      riotslikely_numeric_dk_sp,
      levels = 1:6,
      labels = c(
        "very_likely",
        "quite_likely",
...
prior_riots <- c(
  prior(normal(0, 2),   class = "Intercept", dpar = "mudontknow"),
  prior(normal(0, 2.5), class = "b",         dpar = "mudontknow"),
  prior(student_t(3, 0, 2.5), class = "sd",  dpar = "mudontknow")
)
...
saveRDS(model_1, "riots_likely_model.rds")
