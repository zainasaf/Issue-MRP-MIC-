...
polldata <- polldata  %>% 
  mutate(
    head_of_state_num = case_when(
      head_of_state == "Britain should have an elected politician, such as a President, as the head of state" ~ 1,
      head_of_state == "Britain should continue to have a monarch as the head of state" ~ 0,
      TRUE ~ NA_real_
    ))
...
overall_head_of_state <- polldata %>% group_by(head_of_state_num) %>% count()
overall_head_of_state$pct <- 100*overall_head_of_state$n/nrow(polldata)
...
model_1 <- brm(
  formula = formula_1,
  data = head_of_state_data,
  family = bernoulli(link = "logit"),
  seed = 320,
  silent = 0,
  iter = iter,
  prior = prior_head_of_state,
  chains = chains,
...
saveRDS(model_1, "head_ofstate_model.rds")
