...
fit <- readRDS("gbnews_bias_model.rds")


try(dir.create("age_level_gbnews"))
try(dir.create("edu_level_gbnews"))
try(dir.create("ethn_level_gbnews"))
try(dir.create("sex_level_gbnews"))
try(dir.create("constituency_gbnews"))
try(dir.create("age_sex_level_gbnews"))
...
for(j in 1:nrow(const_names)){  
  ...
  write.csv(pred_age,paste0("age_level_gbnews/gbnews_bias_preds_",const_name_i,".csv"))
  write.csv(pred_edu,paste0("edu_level_gbnews/gbnews_bias_preds_",const_name_i,".csv"))
  write.csv(pred_eth,paste0("ethn_level_gbnews/gbnews_bias_preds_",const_name_i,".csv"))
  write.csv(pred_sex,paste0("sex_level_gbnews/gbnews_bias_preds_",const_name_i,".csv"))
  write.csv(overall_pred,paste0("constituency_gbnews/gbnews_bias_preds_",const_name_i,".csv"))
  write.csv(pred_age_sex,paste0("age_sex_level_gbnews/gbnews_bias_preds_",const_name_i,".csv"))
  
  #  tictoc::toc()
}

const_files <- list.files(path="constituency_gbnews/", pattern="\\.csv", full.names=TRUE)
const_dat <- lapply(const_files, \(x)read_csv(x))
const_dat_comb <- bind_rows(const_dat) %>% select(-1)
write.csv(const_dat_comb, "constituency_gbnews/gbnews_bias_constituency_combined.csv")

age_files <- list.files(path="age_level_gbnews/", pattern="\\.csv", full.names=TRUE)
age_dat <- lapply(age_files, \(x)read_csv(x))
age_dat_comb <- bind_rows(age_dat) %>% select(-1)
write.csv(age_dat_comb, "age_level_gbnews/gbnews_bias_age_combined.csv")

edu_files <- list.files(path="edu_level_gbnews/", pattern="\\.csv", full.names=TRUE)
edu_dat <- lapply(edu_files, \(x)read_csv(x))
edu_dat_comb <- bind_rows(edu_dat) %>% select(-1)
write.csv(edu_dat_comb, "edu_level_gbnews/gbnews_bias_edu_combined.csv")

ethn_files <- list.files(path="ethn_level_gbnews/", pattern="\\.csv", full.names=TRUE)
ethn_dat <- lapply(ethn_files, \(x)read_csv(x))
ethn_dat_comb <- bind_rows(ethn_dat) %>% select(-1)
write.csv(ethn_dat_comb, "ethn_level_gbnews/gbnews_bias_ethn_combined.csv")

sex_files <- list.files(path="sex_level_gbnews/", pattern="\\.csv", full.names=TRUE)
sex_dat <- lapply(sex_files, \(x)read_csv(x))
sex_dat_comb <- bind_rows(sex_dat) %>% select(-1)
write.csv(sex_dat_comb, "sex_level_gbnews/gbnews_bias_sex_combined.csv")

age_sex_files <- list.files(path="age_sex_level_gbnews/", pattern="\\.csv", full.names=TRUE)
age_sex_dat <- lapply(age_sex_files, \(x)read_csv(x))
age_sex_dat_comb <- bind_rows(age_sex_dat) %>% select(-1)
write.csv(age_sex_dat_comb, "age_sex_level_gbnews/gbnews_bias_combined.csv")
